  ...
  if (Node.isObjectLiteralExpression(node)) {
    ...
    for (const property of objectLiteralNode.getProperties()) {
      ...
      if (Node.isPropertyAssignment(property)) {
        ...
        let keyname: string;
        try {
          keyname = property.getFirstChildByKind(
        ...
        inferredTypes[keyname] = typeChecker
...
function init_endpoint({ path, method }) {
  if (!endpoints[path]) {
    endpoints[path] = {
      [method]: {}
    };
...
function processFunctionDeclaration(
  ...
  if (allDeclarations !== undefined && allDeclarations.length === 0) {
    ...
    let queryOrBodyParam = payloadNode ? processTypeNode(payloadNode) : 'never';
